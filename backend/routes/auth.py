...
from backend.models.database import create_owner, get_owner_by_email
...
ALLOWED_BUSINESS_TYPES = {"hotel", "restaurant", "tour_guide", "tour_operator", "tour", "other"}
...
def normalize_business_type(value):
    ...
    if text in {"tour guide", "tour_guide", "tourguide"}:
        return "tour_guide"
    if text in {"tour operator", "tour_operator", "touroperator"}:
        return "tour_operator"
    if text in {"hotel", "restaurant", "other"}:
        return text
    return text
        ...
        try:
            ...
            if owner:
                ...
                if password_match:
                    ...
                    print(f"🔍 REDIRECTING TO: {url_for('dashboard.dashboard')}")
